...
class Fleet{
    ...
    // ***************************************************
    Ship* clearHelper(Ship* aShip); //clear helper function
    Ship* insertHelper(const Ship& aShip, Ship*& tracker); //insert helper function
    Ship* removeHelper(Ship* aShip, int id); //remove helper function
    Ship* findMin(Ship* aShip); //find min node helper funtion
    bool findShipHelper(Ship* aShip, int id) const; //find Ship helper function
    bool setStateHelper(Ship* aShip, int id, STATE state); //setState helper 
   
    Ship* rightRightRotation(Ship* aShip);
    Ship* leftLeftRotation(Ship* aShip);
    Ship* rightLeftRotation(Ship* aShip);
    Ship* leftRightRotation(Ship* aShip);

    //Ship* leftRotation(Ship* aShip); //single left rotation
    //Ship* rightRotation(Ship* aShip); //single right rotation
    
    Ship* removeLostHelper(Ship* aShip);
    void listShipsHelper(Ship* aShip) const;
};
#endif
